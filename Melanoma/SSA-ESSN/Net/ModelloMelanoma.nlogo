;; preamble
extensions [rnd]
globals [M C0 p q k1 k2 k3 alpha11 unosuTau alpha8 alpha10 alpha3 alpha1 alpha2 h alpha6 alpha4 unosuTinj alpha5 alpha7 alpha9 time gammatot Einj Abb Etravel Abtravel Es As C N Ag environment]
...
ask AB [set myrate (list  0 0 0 0 0 ) set totrate 0.0 ]
...
set A1 Ecells with [place = Einj AND (TRUE)]
if any? A1 [
  ask A1 [
...
;; transition Delay
set A1 Ecells with [place = Etravel AND (TRUE)]
if any? A1 [
  ask A1 [
    let countInstances 1
    ;; summing up all rates
    set myrate replace-item 5 myrate (countInstances * (unosuTau))
  ]
]
;; transition Amigration
set A1 AB with [place = Abb AND (TRUE)]
if any? A1 [
  ask A1 [
    let countInstances 1
    ;; summing up all rates
...
;; transition Delay2
set A1 AB with [place = Abtravel AND (TRUE)]
if any? A1 [
  ask A1 [
    let countInstances 1
    ;; summing up all rates
    set myrate replace-item 2 myrate (countInstances * (unosuTau))
  ]
]
;; transition Edeath
set A1 Ecells with [place = Einj AND (TRUE)]
if any? A1 [
  ask A1 [
    let countInstances 1
    ;; summing up all rates
    set myrate replace-item 2 myrate (countInstances * (alpha8))
  ]
]
;; transition Abdeath
set A1 AB with [place = Abb AND (TRUE)]
...
;; transition Esdeath
set A1 Ecells with [place = Es AND (TRUE)]
if any? A1 [
  ask A1 [
    let countInstances 1
    ;; summing up all rates
    set myrate replace-item 0 myrate (countInstances * (alpha8))
  ]
]
;; transition Asdeath
set A1 AB with [place = As AND (TRUE)]
if any? A1 [
  ask A1 [
    let countInstances 1
    ;; summing up all rates
    set myrate replace-item 1 myrate (countInstances * (alpha10))
  ]
]
;; transition NewC
set A1 Ccells with [place = C AND (TRUE)]
if any? A1 [
  ask A1 [
    let countInstances 1
    ;; summing up all rates
    set myrate replace-item 1 myrate (countInstances * (alpha1))
  ]
]
;; transition Ckill
set A1 Ecells with [place = Es AND (TRUE)]
if any? A1 [
  ask A1 [
    ...
    let xe [who] of self
        let A3 Ccells with [place = C AND (TRUE)]
        if any? A3 [
          ask A3 [
            let xc [who] of self
    ...
    ;; summing up all rates
    let ASS count AB with [place = As AND (TRUE)]
    set myrate replace-item 3 myrate (countInstances * (alpha3) *  ((ASS + k2)/(ASS + k3)) * (1 - alpha4) )
...
if any? A1 [
  ask A1 [
    let countInstances 1
    ;; summing up all rates
    set myrate replace-item 0 myrate (countInstances * (alpha2) * ln (count A1) )
  ]
]
;; transition NewN
set A1 ENV with [place = environment AND (TRUE)]
if any? A1 [
  ask A1 [
    let countInstances 1
    ;; summing up all rates
...
;; transition growEs
set A1 Ncells with [place = N AND (TRUE)]
if any? A1 [
  ask A1 [
    let countInstances 0
    let xn [who] of self
    let A2 AGG with [place = Ag AND (TRUE)]
    if any? A2 [
      ask A2 [
        let xag [who] of self
        set countInstances  countInstances + 1
...
if any? A1 [
  ask A1 [
    ...
    ;; summing up all rates
    let ASS count AB with [place = As AND (TRUE)]
      set myrate replace-item 1 myrate (countInstances * (alpha3) *  ((ASS + k2)/(ASS + k3)) * ASS * ( alpha4) )
  ]
]
;; transition AGdeath
set A1 AGG with [place = Ag AND (TRUE)]
if any? A1 [
  ask A1 [
    let countInstances 1
    ;; summing up all rates
...
if any? A1 [
  ask A1 [
    ...
    ;; summing up all rates
    let ASS count AB with [place = As AND (TRUE)]
    set myrate replace-item 3 myrate (countInstances * (alpha7) * (ASS /(ASS + k1)) )
  ]
]
;; transition ESkillsA
set A1 Ecells with [place = Es AND (TRUE)]
if any? A1 [
  ask A1 [
    let countInstances 0
    let xe [who] of self
    let A2 AB with [place = As AND (TRUE)]
    if any? A2 [
      ask A2 [
        let xab [who] of self
        set countInstances  countInstances + 1
...
let allAgents (turtle-set AB AGG Ccells ENV Ecells Ncells )
...
ask chosenAgent [
  ...
  (if-else
  is-a_AB? self [
    ;; select the next action performed by the chosen AB
    let indices  n-values (length myrate) [ i -> i ]
    let pairs (map list indices myrate)
    let nextaction  first rnd:weighted-one-of-list pairs [ [pp] -> last pp ]
    (
      if-else
      nextaction = 0 [
        ;; chosenAgent is leader of Abdeath
        let targetRate random-float (item 0 myrate)
        let AA1 AB with [place = Abb AND (TRUE)]
        if any? AA1 [
          ask AA1 [
      ...
      nextaction = 1 [
        ...
        let AA1 AB with [place = As AND (TRUE)]
        if any? AA1 [
          ask AA1 [
            if NOT bindingSelected [
              let xab [who] of self
      ...
      nextaction = 2 [
        ...
        let AA1 AB with [place = Abtravel AND (TRUE)]
        if any? AA1 [
          ask AA1 [
            if NOT bindingSelected [
              let xab [who] of self
      ...
      nextaction = 3 [
        ...
        if any? AA1 [
          ask AA1 [
            if NOT bindingSelected [
              let xab [who] of self
              let ASS count AB with [place = Es AND (TRUE)]

                    let xe [who] of self
                    set targetRate targetRate - (alpha7) * (ASS /(ASS + k1))
                    if-else targetRate > 0 [ ]
                    [
                      ;; fire this binding
                      set bindingSelected true
  ...
  is-a_AGG? self [
    ...
    (
      if-else
      nextaction = 0 [
        ;; chosenAgent is leader of AGdeath
        let targetRate random-float (item 0 myrate)
        let AA1 AGG with [place = Ag AND (TRUE)]
        if any? AA1 [
          ask AA1 [
            if NOT bindingSelected [
              let xag [who] of self
  ...
  is-a_Ccells? self [
    ...
    (
      ...
      nextaction = 0 [
        ...
        let targetRate random-float (item 0 myrate)
        let AA1 Ccells with [place = C AND (TRUE)]
        if any? AA1 [
          ask AA1 [
            if NOT bindingSelected [
              let xc [who] of self
              set targetRate targetRate  - (alpha2) * ln (count AA1)
              if-else targetRate > 0 [ ]
              [
                ;; fire this binding
                set bindingSelected true
  ...
  is-a_ENV? self [
    ...
    (
      ...
      nextaction = 0 [
        ...
        let targetRate random-float (item 0 myrate)
        let AA1 ENV with [place = environment AND (TRUE)]
        if any? AA1 [
          ask AA1 [
            if NOT bindingSelected [
  ...
  is-a_Ecells? self [
    ...
    (
      ...
      nextaction = 0 [
        ...
        let targetRate random-float (item 0 myrate)
        let AA1 Ecells with [place = Es AND (TRUE)]
        if any? AA1 [
          ask AA1 [
            if NOT bindingSelected [
              let xe [who] of self
      ...
      nextaction = 1 [
        ...
        if any? AA1 [
          ask AA1 [
            if NOT bindingSelected [
              let xe [who] of self
                    let AA3 Ccells with [place = C AND (TRUE)]
                    if any? AA3 [
                      ask AA3 [
                        if NOT bindingSelected [
                          let xc [who] of self
                          let ASS count AB with [place = As AND (TRUE)]
                          set targetRate targetRate -  ( (alpha3) *  ((ASS + k2 )/(ASS + k3 )) * (alpha4) )
                          if-else targetRate > 0 [ ]
                          [
                            ;; fire this binding
                            set bindingSelected true
      ...
      nextaction = 2 [
        ...
        let targetRate random-float (item 2 myrate)
        let AA1 Ecells with [place = Einj AND (TRUE)]
        if any? AA1 [
          ask AA1 [
            if NOT bindingSelected [
              let xe [who] of self
      ...
      nextaction = 4 [
        ;; chosenAgent is leader of ESkillsA
        let targetRate random-float (item 4 myrate)
        let AA1 Ecells with [place = Es AND (TRUE)]
        if any? AA1 [
          ask AA1 [
            if NOT bindingSelected [
              let xe [who] of self
              let AA2 AB with [place = As AND (TRUE)]
              if any? AA2 [
                ask AA2 [
                  if NOT bindingSelected [
                    let xab [who] of self
      ...
      nextaction = 5 [
        ...
        let AA1 Ecells with [place = Etravel AND (TRUE)]
        if any? AA1 [
          ask AA1 [
            if NOT bindingSelected [
              let xe [who] of self
  ...
  is-a_Ncells? self [
    ...
    (
      ...
      nextaction = 0 [
        ...
        let targetRate random-float (item 0 myrate)
        let AA1 Ncells with [place = N AND (TRUE)]
        if any? AA1 [
          ask AA1 [
            if NOT bindingSelected [
              let xn [who] of self
              let AA2 AGG with [place = Ag AND (TRUE)]
              if any? AA2 [
                ask AA2 [
                  if NOT bindingSelected [
                    let xag [who] of self
...
"" ""
PENS
"ccells" 1.0 0 -5298144 true "" "plotxy time count ccells"
"ecells" 1.0 0 -14439633 true "" "plotxy time count ecells"
"AB" 1.0 0 -1184463 true "" "plotxy time count AB"
"AG" 1.0 0 -7500403 true "" "plotxy time count AGG"
"Ncells" 1.0 0 -14070903 true "" "plotxy time count NCells"
